...
<script src="products_db.js?v=13"></script>
      ...
      <div class="form-grid">
        ...
        <div class="field">
          ...
          <select id="f-cat">
            ...
            <option>عروض خاصة</option>
            <option>مماسح مسطحة</option>
            <option>مماسح مايكروفايبر</option>
            <option>قطع غيار</option>
            <option>أحذية رياضية</option>
            <option>حقائب فاخرة</option>
            <option>مجوهرات وساعات</option>
            <option>مماسح بخاخ منزلي</option>
            <option>أجهزة مطبخ</option>
            <option>رياضة ولياقة</option>
            <option>عناية وجمال</option>
            <option>عطور</option>
          </select>
        </div>
        <div class="field">
  ...
  <div class="main-area">
    ...
    <div class="content">
      ...
      <div class="page active" id="page-overview">
        ...
        <div class="stats-grid">
          <div class="stat-card blue">
            ...
            <div class="stat-change up">↑ 18.4% مقارنة بالشهر الماضي</div>
          </div>
      ...
      <div class="page" id="page-orders">
        ...
        <div class="table-card">
          <div style="overflow-x:auto">
            <table>
              <thead><tr>
                <th>رقم الطلبية</th><th>العميل</th><th>المنتجات</th><th>الإجمالي</th><th>الحالة</th><th>التاريخ</th><th>إجراءات</th>
              </tr></thead>
      ...
      <div class="page" id="page-customers">
        <div class="page-header">
          <div>
            ...
            <p class="page-sub">1,247 عميل مسجل</p>
          </div>
        </div>
        <div class="table-card">
          <div style="overflow-x:auto">
            <table>
              <thead><tr><th>الاسم</th><th>البريد الإلكتروني</th><th>المدينة</th><th>الطلبيات</th><th>الإنفاق الكلي</th><th>تاريخ التسجيل</th></tr></thead>
      ...
      <div class="page" id="page-categories">
        <div class="page-header">
          <div>
            <div class="breadcrumb-admin">المحتوى <span>›</span> التصنيفات</div>
            <h1 class="page-title">التصنيفات</h1>
          </div>
          <button class="btn-add" onclick="showToast('💡 قريباً: إضافة تصنيف')">+ تصنيف جديد</button>
        </div>
        <div class="stats-grid" id="cat-grid"></div>
      </div>
      ...
      <div class="page" id="page-analytics">
        ...
        <div class="stats-grid">
          <div class="stat-card blue"><div class="stat-icon">📊</div><div class="stat-val">₪48,320</div><div class="stat-label">مبيعات هذا الشهر</div><div class="stat-change up">↑ 22% عن الشهر السابق</div></div>
          <div class="stat-card green"><div class="stat-icon">🔄</div><div class="stat-val">68%</div><div class="stat-label">معدل تكرار الشراء</div><div class="stat-change up">↑ 5% تحسن</div></div>
          <div class="stat-card yellow"><div class="stat-icon">🛒</div><div class="stat-val">₪385</div><div class="stat-label">متوسط قيمة الطلبية</div><div class="stat-change down">↓ 3% هذا الأسبوع</div></div>
          <div class="stat-card red"><div class="stat-icon">↩️</div><div class="stat-val">2.4%</div><div class="stat-label">معدل الإرجاع</div><div class="stat-change up">↑ أقل من المعدل العام</div></div>
        </div>
...
// ── مصدر البيانات: Store (localStorage مشترك مع الموقع)
let adminProducts = (typeof Store !== 'undefined') ? Store.getProducts() : [];
function getAdminOrders() { return (typeof Store !== 'undefined') ? Store.getOrders() : []; }
...
function showPage(id, el) {
  ...
  if (id === 'orders')   renderOrders((typeof Store !== 'undefined') ? Store.getOrders() : ORDERS);
...
function updateStats() {
  ...
  const storeOrders = (typeof Store !== 'undefined') ? Store.getOrders() : ORDERS;
  const rev = storeOrders.filter(o=>o.status!=='cancelled').reduce((s,o)=>s+(o.total||0),0);
...
function renderBigChart() {
  ...
  const vals = Array.from({length:30},()=>Math.round(800+Math.random()*4000));
  const max = Math.max(...vals);
  el.innerHTML = vals.map((v,i)=>`<div class="bar" style="height:${(v/max)*100}%;background:${i===vals.length-1?'linear-gradient(to top,#10b981,#34d399)':'linear-gradient(to top,rgba(59,130,246,.4),rgba(59,130,246,.8))'}" data-val="₪${v.toLocaleString()}"></div>`).join('');
...
function renderRecentOrders() {
  ...
  body.innerHTML = orders.slice(0,5).map(o => {
    ...
    <tr>
      ...
      <td style="color:var(--text3)">${new Date(o.date).toLocaleDateString('ar-SA')}</td>
    </tr>`;
  }).join('');
}
...
function renderCustomers() {
  ...
  if (!body) return;
  const users = (typeof Store !== 'undefined') ? Store.getUsers().filter(u=>u.role!=='admin') : [];
  if (!users.length) { body.innerHTML = `<tr><td colspan="6" style="text-align:center;color:var(--text3);padding:40px">لا يوجد عملاء مسجلون بعد</td></tr>`; return; }
  const orders = (typeof Store !== 'undefined') ? Store.getOrders() : [];
  body.innerHTML = users.map(c => {
    ...
    return `
    <tr>
      <td><div class="order-customer"><div class="oc-av" style="background:linear-gradient(135deg,#7c3aed,#3b82f6)">${c.name[0]}</div>${c.name}</div></td>
      <td style="color:var(--text3)">${c.email}</td>
      <td>${c.provider === 'google' ? '🔵 Google' : '📧 إيميل'}</td>
      <td style="font-weight:700">${userOrders.length}</td>
      <td style="font-weight:800;color:#fff">₪${spent.toLocaleString()}</td>
      <td style="color:var(--text3)">${new Date(c.createdAt).toLocaleDateString('ar-SA')}</td>
    </tr>`;
  }).join('');
  // Update customers count
  const el = document.querySelector('#page-customers .page-sub');
  if (el) el.textContent = `${users.length} عميل مسجل`;
}
